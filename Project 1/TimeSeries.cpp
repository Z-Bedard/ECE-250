...
/* 
    ...
    Generative AI (ChatGPT o1 Modal as well as 4o) was used to debug my functions. Details will be provided where appropriate. ChatGPT was also used to seperate the program into the three proper files.
*/
...
// --- Load from CSV file ---
void TimeSeries::load(std::string filename) { //Load the CSV file
    ...
    std::cout << "success" << std::endl; //Success message if the file is opened
...
*/
    while (std::getline(CSVdata, datapoint)){
        ...
        while(std::getline(CSVparsed, parsedData, ',')){ //Load valuess into array
            try { //Use a try-catch to see if data is valid (ChatGPT suggested see previous citation)
                double value = std::stod(parsedData);
                if(value > 0){
...
double TimeSeries::mean(TimeSeries &ArrayIn) { //Take in the whole object
    ...
    std::cout << "mean is " << mean << std::endl; //Print mean
    return mean;
}
...
// --- Check if monotonic ---
int TimeSeries::is_monotonic(TimeSeries &ArrayIn) { //Take in object
    int is_monotonic = 0; //Store a value to say which state monotonic is in (is, isn't, no valid data)
    if(ArrayIn.get_size() <= 1){ //Print failure if there's not enough valid data
        is_monotonic = 2;
        return is_monotonic;
    }
    int index = 0; //Label index where there are two unequal values
    while((index < ArrayIn.get_size() - 1) && (ArrayIn.array[index] == ArrayIn.array[index + 1])){
/*
    CITATION: 

    This function was inspired by ChatGPT using the prompt "I want to see if data in an array is increasing or decreasing to create a monotonic function. How would I do that?"
*/
        index++; 
    }
    is_monotonic = 1;
    if(index == ArrayIn.get_size() - 1){
        is_monotonic = 1; //All equal entries is monotonic
        return is_monotonic; 
    }
    if(ArrayIn.array[index] < ArrayIn.array[index + 1]){ //Go to this condition if there is an increasing set of data
        for(int i = index; i < ArrayIn.get_size() - 1; i++){ //If the data starts decreasing break the monotonic condition and return not monotonic
            if(ArrayIn.array[i] > ArrayIn.array[i + 1]){
                is_monotonic = 0;
                break;
            }
        }
    }
    else if(ArrayIn.array[index] > ArrayIn.array[index + 1]){ //Go to this condition if the data is decreasing
        for(int i = index; i < ArrayIn.get_size() - 1; i++){ //If the data starts increasing then break the monotonic condition and return not monotonic
            if(ArrayIn.array[i] < ArrayIn.array[i + 1]){
                is_monotonic = 0;
                break;
            }
