...
CountryList::CountryList() : head(nullptr), HadItem(false) {} //Create a head pointer that's initally set to null to initialize the linked list
...
void CountryList::addNode(NodeData* data){ //Add nodes to the linked list
    ...
    HadItem = true; //Bool value to state if the values have been set or not
}
...
void CountryList::remove(const std::string code){ //Iterate through the linked list with temp pointer to hold the previous value
    ...
    if(head != nullptr && curr->DataID == code){ //Base case in the event the desired node is the first in the list
        temp = head;
        head = head->next;
        delete temp;
        temp = nullptr;
